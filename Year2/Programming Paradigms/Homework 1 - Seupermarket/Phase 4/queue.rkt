...
(define (queue-empty? q)
  (if (and (zero? (queue-size-r q)) (zero? (queue-size-l q)))
      #t
      #f))
...
(define (rotate left right Acc)
  (if (and (null? (cdr right)) (stream-empty? left))
...
(define (enqueue x q)
  (if (< (queue-size-l q) (+ (queue-size-r q) 1))
      (struct-copy queue q
                   [right '()]
                   [size-r 0]
                   [left (rotate (queue-left q) (append (list x) (queue-right q)) '())]
                   [size-l (add1 (+ (queue-size-r q) (queue-size-l q)))]
                   )
      (struct-copy queue q
...
(define (dequeue q)
  (if (< (- (queue-size-l q) 1) (queue-size-r q))
      (struct-copy queue q
                   [right '()]
                   [size-r 0]
                   [left (rotate (stream-rest (queue-left q)) (queue-right q) '())]
