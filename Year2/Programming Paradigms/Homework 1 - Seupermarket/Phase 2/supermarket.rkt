...
(define (remove-first-from-counter C)
  ...
  ;(struct-copy counter C  [tt (-(match C [(counter index tt et queue) tt]) (match C [(counter index tt et queue) et])]
   ;            [queue (reverese (cdr (reverse (match C [(counter index tt et queue) queue]))))]))
...
;(define (actualizare-add pereche lista-case cerere aux)
  ;(cond
    ;((null? lista-case) aux)
    ;((= (match (car lista-case) [(counter index tt et queue) index]) (car pereche)) (actualizare-add pereche (cdr lista-case) cerere (append aux (list (add-to-counter (car lista-case) (car cerere) (car (cdr cerere)))))))
    ;(else (actualizare-add pereche (cdr lista-case) cerere (append aux (list (car lista-case)))))))
...
;Pt remove first
;------------------------------------------------------------------------------------------------
;trebuie verificat si ca lista de clienti sa fie goala
; poate et sa nu fie 0 si totusi lista sa fie goala

(define (min-et-modif lista-case)
  (min-et-modif-helper lista-case (cons 0 1000)))

(define (min-et-modif-helper lista-case pereche)
  (cond
    ((null? lista-case) pereche)
    ((and (< (cdr (create-pereche (car lista-case) afla-et)) (cdr pereche)) (not (equal? (cdr (create-pereche (car lista-case) afla-et)) 0)) (not (null? (match (car lista-case) [(counter index tt et queue) queue]))))
         (min-et-modif-helper (cdr lista-case) (create-pereche (car lista-case) afla-et)))
    (else (min-et-modif-helper (cdr lista-case) pereche))))

(define (actualizare-remove-first fast-counters slow-counters)
  (if (<= (cdr (min-et-modif fast-counters)) (cdr (min-et-modif slow-counters)))
      (list (remove-first-from-list fast-counters (min-et-modif fast-counters) '()) slow-counters)
      (list fast-counters (remove-first-from-list slow-counters (min-et-modif slow-counters) '()))))
...
#|
(define (min-et-modif lista-case)
  (min-et-modif-helper lista-case (create-pereche (car lista-case) afla-et)))
...
(define (remove-first-from-list lista-case pereche)
  (foldl (λ (x aux)
           (if (= (match x [(counter index tt et queue) index]) (car pereche))
               (append aux (list (remove-first-from-counter x )))
               (append aux (list x))))
                  '()
                  lista-case))
|#
