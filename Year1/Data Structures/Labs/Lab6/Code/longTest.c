...
int isBST(BSTree* t, BSTNode* node){
	if (node == NULL)  
    		return 1;  
      

  	if (node->l!=NULL && maximum(t,node->l)->elem > node->elem)  
    		return 0;  
      
  
  	if (node->r != NULL && minimum(t,node->r)->elem < node->elem)  
    		return 0;  
    
  
  	if (!isBST(t,node->l) || !isBST(t,node->r))  
    		return 0;  
